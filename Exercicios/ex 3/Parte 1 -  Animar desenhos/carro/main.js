...
function drawScene() {
  ...
  // atualize a posição e rotação das rodas
  carX += 0.01;
  wheelRotation += 0.01
  ...
  // redesenha as rodas com rotação
  drawRotatedWheel(-0.35 + carX, -0.5, 0.2, wheelRotation); // Roda esquerda
  drawRotatedWheel(0.35 + carX, -0.5, 0.2, wheelRotation);  // Roda direita
...
function drawRotatedWheel(centerX, centerY, radius, rotationAngle) {
  gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);
  setRotatedCircleVertices(gl, n, radius, centerX, centerY, rotationAngle);
  gl.bindBuffer(gl.ARRAY_BUFFER, colorBuffer);
  setCircleColor(gl, n, [0.1, 0.1, 0.1]);
  gl.drawArrays(gl.TRIANGLES, 0, 3 * n);
}

function setRotatedCircleVertices(gl, n, radius, centerX, centerY, angle) {
  let vertexData = [];
  for (let i = 0; i < n; i++) {
    const theta = (i * 2 * Math.PI) / n + angle; // Adiciona o ângulo de rotação
    const x = centerX + radius * Math.cos(theta);
    const y = centerY + radius * Math.sin(theta);
    vertexData.push(centerX, centerY, x, y);
    const nextTheta = ((i + 1) * 2 * Math.PI) / n + angle;
    vertexData.push(
